...
export function renderStaticSVG(profiles: SoilProfileCollection, options: StaticRenderOptions): string {
    ...
    if (mode === 'depth') {
        svg += renderDepthMode(profiles, width, height, theme);
    } else if (mode === 'texture') {
        svg += renderTextureMode(profiles, width, height, theme);
    } else if (mode === 'properties') {
        svg += renderPropertyMode(profiles, width, height, theme);
    } else if (mode === 'thumbnail') {
        svg += renderThumbnailMode(profiles, width, height, theme);
    }
...
function renderThumbnailMode(profiles: SoilProfileCollection, width: number, height: number, theme: any): string {
    const padding = 5;
...
function renderDepthMode(profiles: SoilProfileCollection, width: number, height: number, theme: any): string {
    const padding = 40;
    const maxDepth = profiles.getMaxDepth() || 100;
    const depthScale = (height - padding * 2) / maxDepth;
...
function renderTextureMode(profiles: SoilProfileCollection, width: number, height: number, theme: any): string {
    const padding = 40;
    const maxDepth = profiles.getMaxDepth() || 100;
    const depthScale = (height - padding * 2) / maxDepth;
    ...
    profile.horizons.forEach(hz => {
        ...
        [
            ...
            svg += `<circle cx="${point.x}" cy="${y}" r="4" fill="${point.color}" stroke="#333" stroke-width="0.5">`;
...
function renderPropertyMode(profiles: SoilProfileCollection, width: number, height: number, theme: any): string {
    const padding = 40;
    const maxDepth = profiles.getMaxDepth() || 100;
    const depthScale = (height - padding * 2) / maxDepth;
    const profile = profiles.profiles[0];
    ...
    profile.horizons.forEach(hz => {
        ...
        if (hz.ph !== undefined) {
            ...
            svg += `<rect x="${marginLeft + 20}" y="${y1}" width="${offset}" height="${y2 - y1}" fill="${color}" opacity="0.7" stroke="${theme.textColor}" stroke-width="1">`;
